...
# Connectivity estimators - corr and partial
correlation_measure_bi = ConnectivityMeasure(kind='correlation') 
correlation_measure_part = ConnectivityMeasure(kind='partial correlation') 
...
for i in ts_files_mid:
  ...
  ts_mid.append(pd.read_csv(os.path.join(ts_path_mid,  i), usecols=range(1,ncols),skiprows=1, header= None))
                    ...
                    #### Calculaitng Connectome Matricies ####

# Calculating Correlation 
  ## Resting
cor_mat_bi = []
for i in ts:
  cor_mat_bi.append(correlation_measure_bi.fit_transform([np.array(i)])[0])
...
len(cor_mat_bi) == 578
    ...
    # writing
# for i in range(len(cor_mat_bi)):
#  pd.DataFrame(cor_mat_bi[i]).to_csv(r"C:\Users\wintersd\OneDrive - The University of Colorado Denver\1 Publications\ABCD DATA\correlation_matrix_rest_CORR\sp_{}.csv".format(ids[i]))
...
with parallel_backend('threading', n_jobs=12): 
  for i in ts_mid:
    cor_mid_mat_bi.append(correlation_measure_bi.fit_transform([np.array(i)])[0])


for i in cor_mid_mat_bi:
  i.shape


len(cor_mid_mat_bi) == 489


    # writing
# for i in range(len(cor_mid_mat_bi)):
#  pd.DataFrame(cor_mid_mat_bi[i]).to_csv(r"C:\Users\wintersd\OneDrive - The University of Colorado Denver\1 Publications\ABCD DATA\correlation_matrix_MID_CORR\sp_{}.csv".format(ids[i]))
...
cor_mat_part = []
for i in ts:
  cor_mat_part.append(correlation_measure_part.fit_transform([np.array(i)])[0])


for i in cor_mat_part:
  i.shape


len(cor_mat_part) == 578


    # writing
# for i in range(len(cor_mat_part)):
#  pd.DataFrame(cor_mat_part[i]).to_csv(r"C:\Users\wintersd\OneDrive - The University of Colorado Denver\1 Publications\ABCD DATA\correlation_matrix_rest_PART_CORR\sp_{}.csv".format(ids[i]))
...
with parallel_backend('threading', n_jobs=-1): 
  for i in ts_mid:
    cor_mid_mat_part.append(correlation_measure_part.fit_transform([np.array(i)])[0])


for i in cor_mid_mat_part:
  i.shape


len(cor_mid_mat_part) == 489

   # writing 
# for i in range(len(cor_mid_mat_part)):
#  pd.DataFrame(cor_mid_mat_part[i]).to_csv(r"C:\Users\wintersd\OneDrive - The University of Colorado Denver\1 Publications\ABCD DATA\correlation_matrix_MID_PART_CORR\sp_{}.csv".format(ids[i]))
...
with parallel_backend('threading', n_jobs=-1): 
  for i in ts:
    cor_mat_ddc.append(dcov_relu(i,0.1))
...
with parallel_backend('threading', n_jobs=-1): 
  for i in ts_mid:
    mid_mat_ddc.append(dcov_relu(i,0.1)) 
...
#  pd.DataFrame(mid_mat_ddc[i]).to_csv(r"D:\CU OneDrive Sync\1 Publications\ABCD DATA\correlation_matrix_MID_DDC\sp_{}.csv".format(ids[i]))
                        ...
                        ### loading data ###
# After calculation we can just load the data to reduce time

  # loading correlation matricies

cor_bi_files = os.listdir(r"C:\Users\wintersd\OneDrive - The University of Colorado Denver\1 Publications\ABCD DATA\correlation_matrix_rest_CORR")
len(cor_bi_files) #578
cor_mat_bi = []
cor_bi_path = r"C:\Users\wintersd\OneDrive - The University of Colorado Denver\1 Publications\ABCD DATA\correlation_matrix_rest_CORR"
for i in cor_bi_files:
  ncols = len(open(os.path.join(cor_bi_path,  i)).readline().split(','))
  cor_mat_bi.append(pd.read_csv(os.path.join(cor_bi_path,  i), usecols=range(1,ncols),skiprows=1, header= None))


cor_bi_mid_files = os.listdir(r"C:\Users\wintersd\OneDrive - The University of Colorado Denver\1 Publications\ABCD DATA\correlation_matrix_MID_CORR")
len(cor_bi_mid_files) #578
cor_mid_mat_bi = []
cor_bi_mid_path = r"C:\Users\wintersd\OneDrive - The University of Colorado Denver\1 Publications\ABCD DATA\correlation_matrix_MID_CORR"
for i in cor_bi_mid_files:
  ncols = len(open(os.path.join(cor_bi_mid_path,  i)).readline().split(','))
  cor_mat_mid_bi.append(pd.read_csv(os.path.join(cor_bi_mid_path,  i), usecols=range(1,ncols),skiprows=1, header= None))


  # loading partial matricies 

cor_part_files = os.listdir(r"C:\Users\wintersd\OneDrive - The University of Colorado Denver\1 Publications\ABCD DATA\correlation_matrix_rest_PART_CORR")
len(cor_part_files) #578
cor_mat_part = []
cor_part_path = r"C:\Users\wintersd\OneDrive - The University of Colorado Denver\1 Publications\ABCD DATA\correlation_matrix_rest_PART_CORR"
for i in cor_part_files:
  ncols = len(open(os.path.join(cor_part_path,  i)).readline().split(','))
  cor_mat_part.append(pd.read_csv(os.path.join(cor_part_path,  i), usecols=range(1,ncols),skiprows=1, header= None))

cor_part_mid_files = os.listdir(r"C:\Users\wintersd\OneDrive - The University of Colorado Denver\1 Publications\ABCD DATA\correlation_matrix_MID_PART_CORR")
len(cor_part_mid_files) #578
cor_mid_mat_part = []
cor_part_mid_path = r"C:\Users\wintersd\OneDrive - The University of Colorado Denver\1 Publications\ABCD DATA\correlation_matrix_MID_PART_CORR"
for i in cor_part_mid_files:
  ncols = len(open(os.path.join(cor_part_mid_path,  i)).readline().split(','))
  cor_mat_mid_part.append(pd.read_csv(os.path.join(cor_part_mid_path,  i), usecols=range(1,ncols),skiprows=1, header= None))
...
# Calculating hitting matrix with correlation matricies 
...
with parallel_backend('threading', n_jobs=12): 
  for i in cor_mat_bi:
    hit_mat_bi.append(hitting_matrix(i))


for i in hit_mat_bi:
  i.shape

len(hit_mat_bi) == 578 # TRUE

  # writing
# for i in range(len(hit_mat_bi)):
#  pd.DataFrame(hit_mat_bi[i]).to_csv(r"C:\Users\wintersd\OneDrive - The University of Colorado Denver\1 Publications\ABCD DATA\hitting_matrix_rest_CORR\sp_{}.csv".format(ids[i]), header= False, index = False)
# 
...
with parallel_backend('threading', n_jobs=12): 
  for i in cor_mid_mat_bi:
    hit_mid_mat_bi.append(hitting_matrix(i))


for i in hit_mid_mat_bi:
  i.shape

len(hit_mid_mat_bi) == 489 # TRUE

any_values_bi = []
for i in range(len(hit_mid_mat_bi)):
  any_values_bi.append(pd.DataFrame(hit_mid_mat_bi[i]).notnull().values.any())

# pd.DataFrame({"id":ids, "any_not_null":any_values_bi})

(pd.DataFrame(any_values_bi)[0] == False).any()
np.sum(pd.DataFrame(any_values_bi)[0] == False)
  ## A total of 0 are  NULL 


  # writing
# for i in range(len(hit_mid_mat_bi)):
#  pd.DataFrame(hit_mid_mat_bi[i]).to_csv(r"C:\Users\wintersd\OneDrive - The University of Colorado Denver\1 Publications\ABCD DATA\hitting_matrix_MID_CORR\sp_{}.csv".format(ids[i]), header= False, index = False)


# Calculating hitting matrix with partial matricies
hit_mat_part = []
from joblib import parallel_backend
with parallel_backend('threading', n_jobs=12): 
  for i in cor_mat_part:
    hit_mat_part.append(hitting_matrix(i))

for i in hit_mat_part:
  i.shape

len(hit_mat_part) == 578 # TRUE

any_values_part = []
for i in range(len(hit_mat_part)):
  any_values_part.append(pd.DataFrame(hit_mat_part[i]).notnull().values.any())

# pd.DataFrame({"id":ids, "any_not_null":any_values_part})

(pd.DataFrame(any_values_part)[0] == False).any() # there are no NULL matricies

  # writing
# for i in range(len(hit_mat_part)):
#  pd.DataFrame(hit_mat_part[i]).to_csv(r"C:\Users\wintersd\OneDrive - The University of Colorado Denver\1 Publications\ABCD DATA\hitting_matrix_rest_PART_CORR\sp_{}.csv".format(ids[i]), header= False, index = False)
...
with parallel_backend('threading', n_jobs=12): 
  for i in cor_mid_mat_part:
    hit_mid_mat_part.append(hitting_matrix(i))

for i in hit_mid_mat_part:
  i.shape

len(hit_mid_mat_part) == 489 # TRUE

  # writing
# for i in range(len(hit_mid_mat_part)):
#  pd.DataFrame(hit_mid_mat_part[i]).to_csv(r"C:\Users\wintersd\OneDrive - The University of Colorado Denver\1 Publications\ABCD DATA\hitting_matrix_MID_PART_CORR\sp_{}.csv".format(ids[i]), header= False, index = False)




# Calculating hitting matrix with the DDC matricies
hit_rest_mat_ddc = []
from joblib import parallel_backend
with parallel_backend('threading', n_jobs=-1):
